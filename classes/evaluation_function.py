...
if TYPE_CHECKING:
    ...
    from classes.rule import Context, Rule  # circular import  # noqa: TC004,RUF100
...
if TYPE_CHECKING:
    ...
    _W = Union[
        _Wrapped[Concatenate["Rule", "Context", ...], Any, Concatenate["Rule", "Context", ...], Any],
...
class ConditionFunction(Generic[_CP, _CH]):
    ...
    def __new__(
        cls,
        first_argument: Optional[Annotated[str, "name"]] = None,
        name: str = "ConditionFunction",
        *,
        truthy: Literal[False] = False,
        use_self: Optional[bool] = None,
    ) -> Callable[[CallableCondition[RuleT, _P, _H]], ConditionFunction[_P, _H]]: ...
    ...
    def __new__(
        ...
        *,
        truthy: Literal[True],
        use_self: Optional[bool] = None,
    ) -> Callable[[CallableCondition[RuleT, _P, Any]], ConditionFunction[_P, bool]]: ...

    @overload
    def __new__(
        cls,
        first_argument: CallableCondition[RuleT, _CP, _CH],
        name: str = "ConditionFunction",
        *,
        truthy: bool = False,
        use_self: Optional[bool] = None,
    ...
    def __new__(
        ...
        *,
        ...
        assert isinstance(first_argument, (Callable, staticmethod)), (
            f"First argument must be a callable, not {type(first_argument)}"
        )
        # @ConditionFunction or ConditionFunction(function)
        return super().__new__(cls)  # new instance

    if READTHEDOCS and not TYPE_CHECKING:
        __new__.__annotations__["first_argument"] = Optional["str(name)" | AnyCallableCondition]  # noqa: F821, TC010
    ...
    def __call__(self, ctx: Union["Rule", "Context"], *args: _CP.args, **kwargs: _CP.kwargs) -> _CH:  # pyright: ignore[reportInconsistentOverload]
        ...
        if not isinstance(ctx, Context):
            ...
            assert isinstance(ctx, Context), (
                f"This should not happen: In the method case the argument must be a Context object, not {type(ctx)}"
            )
    ...
    def __get__(self, instance: "Optional[Rule]", objtype: Optional["type[Rule]"] = None) -> "Self | partial[_CH]":
        ...
        return partial(
            self, instance
        )  # NOTE: This fixes "ctx" to instance in __call__, the real "ctx" in __call__ is provided through *args
    ...
    def copy(self, copy_actions: bool = False):
        ...
        self.__class__.__init__(
            instance, self.evaluation_function, self.name, truthy=self.truthy, use_self=self.use_self
        )
        if copy_actions:
    ...
    @overload
    def _check_action(
        self, action_function: CallableT, key: Hashable, *, use_self: Optional[bool] = None
    ) -> CallableT: ...

    @overload
    def _check_action(
        self,
        action_function: Callable[_P, _T],
        key: Hashable,
        *,
        use_self: Optional[bool] = None,
    ...
    def _check_action(
        ...
        **preset_kwargs: _P.kwargs,
        ...
        if action_function.__name__ in ConditionFunction._INVALID_NAMES:
            msg = (
                "When using ConditionFunction.add_action, the action function's name "
                f"may not be in {ConditionFunction._INVALID_NAMES}, got '{action_function.__name__}'."
            )
            raise ValueError(msg)
        if key in self.actions:
            print(
                "Warning: Overwriting already registered action",
                self.actions[key],
                "with key",
                f"'{key}'",
                "in",
        ...
        if hasattr(action_function, "use_self") and action_function.use_self is not use_self:  # pyright: ignore[reportFunctionMemberAccess]
            print(
                "Warning: Registering action:",
                action_function.__name__,
                "with a different use_self value than before. old:",
                action_function.use_self,  # pyright: ignore[reportFunctionMemberAccess]
                "new:",
                use_self,
            )
    ...
    if TYPE_CHECKING:
        ...
        class _RegisterActionDecorator:
            ...
            @overload
            def __call__(
                self, func: Callable[Concatenate[RuleT, Context, ...], _T]
            ) -> Callable[[RuleT, Context], _T]: ...
    ...
    def register_action(
        self, key: typing.Hashable = True, *, use_self: Optional[bool] = None, **kwargs: _P.kwargs
        ...
        def decorator(action_function: AnyCallableAction) -> AnyCallableAction:
            checked_action = self._check_action(action_function, key, use_self=use_self, **kwargs)
    ...
    def _register_action_directly(
        self, action_function: CallableT, key: Hashable = True, *, use_self: Optional[bool] = None, **kwargs: _P.kwargs
    ) -> CallableT:
        """
        - Case 1
    ...
    @classmethod
    def AND(
        cls, func1: "ConditionFunction[_CP,  _CH]", func2: "ConditionFunction[_CP, _H]"
    ) -> "ConditionFunction[_CP, _CH | _H]":
...
class ActionFunction(ConditionFunction[_P, _T]):
    ...
    def __init__(
        self,
        action_function: CallableAction[RuleT, _P, _T],
        name: str = "ActionFunction",
        *,
        use_self: Optional[bool] = None,
    ):
