...
def _use_temporary_config(
    func: Callable[Concatenate[_Rule, "Context", Optional[Dict[str, Any]], _P], _T],
    ...
    @wraps(func)
    def wrapper(
        self: _Rule, ctx: Context, overwrite: Optional[Dict[str, Any]] = None, *args: _P.args, **kwargs: _P.kwargs
    ) -> _T:
        ...
        OmegaConf.set_readonly(
            self.self_config, False
        )  # The Rule's settings should still be dynamic; expected in overwrite
        try:
...
class _CountdownRule:
    ...
    def is_ready(self) -> bool:
        ...
        return (
            self.cooldown == 0 and self.enabled and not self.blocked
        )  # Note: uses property getters. Group aware for GroupRules
...
_GroupInstanceValues = TypedDict(
    "_GroupInstanceValues", {"cooldown": int, "max_cooldown": int, "instances": "WeakSet[_GroupRule]"}
)
...
class _GroupRule(_CountdownRule):
    ...
    def set_my_group_cooldown(self, value: Optional[int] = None):
        ...
        if self.group is None:
            ...
            return
        if value is None:
            self._group_instances[self.group]["cooldown"] = self._group_instances[self.group][
                "max_cooldown"
            ]  # set to max
        else:
    ...
    __filter_not_ready_instances: Callable[["_CountdownRule"], bool] = (
        lambda instance: instance.group is None and instance._cooldown > 0
    )  # pylint: disable=line-too-long # type: ignore[attr-defined]
    """
...
class Rule(_GroupRule):
    ...
    false_action: Annotated[
        CallableAction[Self, [], Any], "attribute not available on instance -> merged into `actions`"
    ]
    ...
    def __new__(
        cls,
        phases: Optional[Union[Phase, Iterable[Phase], "type[Any]", Self, str, None]] = None,
        ...
        **kwargs: Any,
        ...
        if isclass(phases):
            if issubclass(phases, _CountdownRule):
                raise ValueError(
                    "When using @Rule the class may not be a subclass of Rule. Do not inherit from rule or subclass from Rule instead with the decorator."
                    "Do not do:\n\t@Rule\n\tclass MyRule(>>Rule<<): ...\n"
                )
            ...
            new_decorated_class = type(
                decorated_class.__name__, (cls,), decorated_class.__dict__.copy(), _init_by_decorator=True, **kwargs
            )  # > calls init_subclass; copy() for correct type!
            if TYPE_CHECKING:
        ...
        if isinstance(phases, str):
            try:
                ...
                print(
                    "ERROR: If you want to initialize a rule be sure that you pass a Phase object as the first argument."
    ...
    def __init__(
        ...
        *,
        ...
        if action is not None and actions is not None:
            try:
                ...
                raise ValueError(
                    ...
                    "- other actions are currently not supported."
                ) from e
        ...
        default_self_config: "RuleConfig" = cast(
            "RuleConfig", getattr(self, "self_config", getattr(self, "SelfConfig", {}))
        )
        ...
        if not isinstance(default_self_config, DictConfig):
            default_self_config = cast(
                "RuleConfig", OmegaConf.create(default_self_config, flags={"allow_objects": True})
            )  # type: ignore
        if self_config:
    ...
    def __init_subclass__(cls, _init_by_decorator: bool = False, metarule: bool = False):
        ...
        if hasattr(cls, "phases") and hasattr(cls, "phase") and cls.phases and cls.phase:
            raise ValueError(
                f"Both 'phases' and 'phase' are set in class {cls.__name__}. Use only one. %s, %s"
                % (cls.phases, cls.phase)
            )
        ...
        if hasattr(cls, "condition"):
            ...
            if hasattr(cls.condition, "use_self") and cls.condition.use_self is not None:  # pyright: ignore[reportUnknownMemberType, reportUnknownArgumentType, reportFunctionMemberAccess,  reportAttributeAccessIssue]
                ...
                if params >= 2:
                    if params > 2:
                        logger.warning(
                            f"Rule {getattr(cls.condition, '__name__', cls.condition)}"  # pyright: ignore[reportUnknownMemberType, reportUnknownArgumentType]
            ...
            if condition_as_method:
                ...
                logger.debug(
                    "Implementing %s as function(ctx) without a self argument "
            ...
            if hasattr(cls.condition, "actions") and cls.condition.actions:  # type: ignore[attr-defined]
                if hasattr(cls, "actions") and cls.actions:
                    msg = (
                        f"Class {cls.__name__} already has an 'actions' attribute. "
        ...
        def partial_init(self: Self, phases: Optional[Iterable[Phase]] = None, *args, **kwargs: Rule._InitParameters):  # pyright: ignore[reportMissingParameterType, reportUnknownParameterType]
            ...
            try:
                ...
                if "missing" in str(e):
                    logger.error(
                        "Class %s has likely missing attributes that cannot be passed to init. Check if all required attributes are set in the class definition.",
                        cls.__name__,
                    )
                raise
    ...
    def __init_from_mapping(self, source: "_InitParametersComplete"):  # pyright: ignore[reportUnusedFunction]
        ...
        self.__init__(
            source.get("phases", source.get("phase")),
            condition,
            source.get("action"),
    ...
    def execute_phase(
        self, phase: Phase, *, prior_results: Any = None, update_controls: Optional[carla.VehicleControl] = None
    ):
        """
        Attention:
        ...
        """
        try:
            self._ctx.agent.execute_phase(
                phase=phase,  # pyright: ignore[reportOptionalMemberAccess]
    ...
    def __call__(
        ...
        *,
        ...
        if (
            not ignore_phase and ctx.agent.current_phase not in self.phases
        ):  # NOTE: This is currently never False as checked in execute_phase and the agents dictionary.
        ...
        try:
            ...
            if result in self.actions:
                ...
                action_result = self.actions[result](
                    ctx
                )  # todo allow priority, random chance  # pyright: ignore[reportCallIssue]
    ...
    def __str__(self) -> str:
        try:
            if isinstance(self.condition, partial):
                return (
                    self.__class__.__name__ + f"(description='{self.description}', "
            ...
            return (
                self.__class__.__name__ + "(Error in condition.__str__: Rule has not been initialized correctly. "
    ...
    def __init__(
        ...
        *,
        ...
        if rules is None:  # type: ignore
            logger.warning(
                "Warning: No rules passed to %s: %s. You can still add rules to the rules attribute later.",
                self.__class__.mro()[1].__name__,
...
class RandomRule(MultiRule, metarule=True):
    ...
    def __init_by_decorating_class(self, cls: "RandomRule"):  # pyright: ignore[reportUnusedFunction]
        ...
        self.__init__(
            phases,
            cls.rules,
            repeat_if_not_applicable=cls.repeat_if_not_applicable,
    ...
    def evaluate_children(self, ctx: Context, overwrite: Optional[Dict[str, Any]] = None) -> Any:
        ...
        while rules:
            ...
            try:
                result = rule(
                    ctx, overwrite, ignore_phase=self.ignore_phase
                )  # NOTE: Context action/evaluation results only store result of LAST rule
            except DoNotEvaluateChildRules:
                return None
...
class BlockingRule(Rule, metarule=True):
    ...
    def __init_by_decorating_class(self, cls: "BlockingRule"):  # pyright: ignore[reportUnusedFunction]
        ...
        self.__init__(
            ...
            false_action=getattr(cls, "false_action", None),
    ...
    def _begin_tick(self, ctx: Context):
        ...
        frame = None
        if (
            self._gameframework and self._gameframework.launch_config.handle_ticks
        ):  # i.e. no scenario runner doing it for us
    ...
    def update_world(
        self, ctx: Context, *, execute_phases: Union[bool, Container[Phase]] = True
    ) -> "carla.VehicleControl | None":
        """
        ...
        if self.ticks_passed > self.MAX_TICKS:
            logger.info(
                "Rule %s has passed its max_ticks %s, calling max_tick_callback and unblocking it", self, self.MAX_TICKS
            )
    ...
    def __call__(
        ...
        *,  # Kwargs from Rule.__call__
        ...
        if self in ctx.agent._active_blocking_rules:  # pyright: ignore[reportPrivateUsage]
            logger.warning(
                "Rule %s is already blocking the agent, this is a recursive call. Not executing the rule.", self
            )
    ...
    def evaluate(
        self, ctx: Context, overwrite: Optional[Dict[str, Any]] = None
    ) -> Union[bool, Hashable, Literal[RuleResult.NO_RESULT]]:
