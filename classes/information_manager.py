...
class InformationManager:
    ...
    def _check_state(state: AgentState):  # pyright: ignore[reportSelfClsParameterName, reportGeneralTypeIssues]
        ...
        def wrapper(
            func: Callable[Concatenate[Self, _P], _T],
        ) -> Callable[Concatenate[Self, _P], _T]:  # -> _Wrapped[Callable[Concatenate[Self, _P], Any], bool | Non...:
            @wraps(func)
    ...
    def detect_next_traffic_light(self):
        ...
        if not self.relevant_traffic_light or tlight_distance > self.relevant_traffic_light_distance * 1.01:
            ...
            if self.relevant_traffic_light and tlight_distance > self.relevant_traffic_light_distance * 1.01:
                logger.debug(
                    "Traffic light distance increased %s, did slow update.", self.relevant_traffic_light_distance
                )
    ...
    def tick(self):
        ...
        self.live_info.current_location = current_loc = CarlaDataProvider.get_location(
            self._vehicle
        )  # NOTE: is None if past run not cleaned # noqa: E501 # type: ignore
        # Only exact waypoint. TODO: update in agent
        # Comment should be visible in traceback.
        current_waypoint = cast(
            "carla.Waypoint", CarlaDataProvider.get_map().get_waypoint(current_loc)
        )  # NOTE: Might throw error if past run was not cleaned; or the world did not tick yet. # noqa: E501 # pyright: ignore[reportCallIssue, reportArgumentType]
        ...
        def dist(v: carla.Actor):
            if not v.is_alive:
                ...
                return v_filter_dist  # filter out
            return v.get_location().distance(current_loc)  # pyright: ignore[reportArgumentType]

        # Filter nearby
        # Vehicles
        v_filter_dist = self._agent.config.obstacles.nearby_vehicles_max_distance
        self.vehicles_nearby: List[carla.Vehicle] = []
        for v in self.vehicles:
            if v.id != self._vehicle.id and dist(v) < v_filter_dist:
        ...
        self.traffic_lights_nearby = [
            tl
            for tl in InformationManager.get_traffic_lights()
    ...
    def global_tick(frame: Optional[int] = None) -> None:
        ...
        for actor_id in CarlaDataProvider._carla_actor_pool.copy():
            try:
                ...
                if actor is None or not actor.is_alive:  # pyright: ignore[reportUnnecessaryComparison]
                    logger.debug("Detected dead actor in the pool. %s", (actor.id, actor.type_id, actor.attributes))  # pyright: ignore[reportUnknownMemberType, reportUnknownArgumentType]
        ...
        InformationManager.obstacles = (
            InformationManager.walkers + InformationManager.static_obstacles + InformationManager.vehicles
        )
