...
CameraBlueprints: Final = {
    ...
    "Camera Depth (Logarithmic Gray Scale)": CameraBlueprint(
        "sensor.camera.depth", cc.LogarithmicDepth, "Camera Depth (Logarithmic Gray Scale)"
    ),
    "Camera Semantic Segmentation (Raw)": CameraBlueprint(
        "sensor.camera.semantic_segmentation", cc.Raw, "Camera Semantic Segmentation (Raw)"
    ),
    "Camera Semantic Segmentation (CityScapes Palette)": CameraBlueprint(
...
class CameraManager(MockDummy.CanBeDummy, CustomSensorInterface):
    ...
    def __init__(
        ...
        self._camera_transforms = [
            ...
            (carla.Transform(carla.Location(x=-1.0, y=-1.0 * bound_y, z=0.4 * bound_z)), AttachmentType.Rigid),
        ]
    ...
    def set_sensor(self, index: Optional[int], notify=True, force_respawn=False) -> None:
        ...
        needs_respawn = (
            True if self.index is None else (force_respawn or (self.sensors[index][0] != self.sensors[self.index][0]))
        )
        if needs_respawn:
            ...
            self.sensor = cast(
                "carla.Sensor",
                CarlaDataProvider.get_world().spawn_actor(
    ...
    def _parse_image(weak_self: "weakref.ref[CameraManager]", image: carla.Image):
        ...
        if self.recording and (
            (image.frame % self._frame_interval) == 0 or self.current_frame + self._frame_interval < image.frame
        ):
    ...
    def follow_actor(
        cls_or_self: "Self | type[Self]", actor: Optional[carla.Actor] = None, updater=spectator_follow_actor
    ) -> None:
        """
