...
class BasicAgent:
    ...
    def __init__(self, vehicle, behavior, map_inst=None, grp_inst=None, overwrite_options: dict = {}):
        ...
        #config.obstacles.ignore_traffic_lights = False
        config.obstacles.ignore_traffic_lights
        #config.obstacles.ignore_stop_signs = False
        config.obstacles.ignore_stop_signs
        #config.obstacles.ignore_vehicles = False
        config.obstacles.ignore_vehicles
        #config.unknown.use_bbs_detection = False
        config.unknown.use_bbs_detection
        #config.unknown.sampling_resolution = 2.0
        config.unknown.sampling_resolution
        #config.obstacles.base_tlight_threshold = 5.0  # meters
        config.obstacles.base_tlight_threshold
        #config.obstacles.base_vehicle_threshold = 5.0  # meters
        config.obstacles.base_vehicle_threshold
        #config.obstacles.detection_speed_ratio = 1
        config.obstacles.detection_speed_ratio
        #config.controls.max_brake = 0.5
        config.controls.max_brake
        #config.controls.offset= 0
        config.controls.offset
        ...
        if 'ignore_traffic_lights' in opt_dict:
            config.obstacles.ignore_traffic_lights = opt_dict['ignore_traffic_lights']
        if 'ignore_stop_signs' in opt_dict:
            config.obstacles.ignore_stop_signs = opt_dict['ignore_stop_signs']
        if 'ignore_vehicles' in opt_dict:
            config.obstacles.ignore_vehicles = opt_dict['ignore_vehicles']
        if 'use_bbs_detection' in opt_dict:
            config.unknown.use_bbs_detection = opt_dict['use_bbs_detection']
        if 'sampling_resolution' in opt_dict:
            config.unknown.sampling_resolution = opt_dict['sampling_resolution']
        if 'base_tlight_threshold' in opt_dict:
            config.obstacles.base_tlight_threshold = opt_dict['base_tlight_threshold']
        if 'base_vehicle_threshold' in opt_dict:
            config.obstacles.base_vehicle_threshold = opt_dict['base_vehicle_threshold']
        if 'detection_speed_ratio' in opt_dict:
            config.obstacles.detection_speed_ratio = opt_dict['detection_speed_ratio']
        if 'max_brake' in opt_dict:
            config.controls.max_brake = opt_dict['max_brake']
        if 'offset' in opt_dict:
            config.controls.offset= opt_dict['offset']
        ...
        if grp_inst:
            if isinstance(grp_inst, GlobalRoutePlanner):
                ...
                print("Warning: Ignoring the given map as it is not a 'carla.Map'")
                self._global_planner = GlobalRoutePlanner(self._map, config.unknown.sampling_resolution)
        else:
            self._global_planner = GlobalRoutePlanner(self._map, config.unknown.sampling_resolution)
    ...
    def run_step(self):  # From basic_agent.py
        ...
        max_vehicle_distance = config.obstacles.base_vehicle_threshold + config.obstacles.detection_speed_ratio * vehicle_speed
        affected_by_vehicle, _, _ = self._vehicle_obstacle_detected(vehicle_list, max_vehicle_distance)
        if affected_by_vehicle:
            hazard_detected = True

        # Check if the vehicle is affected by a red traffic light
        max_tlight_distance = config.obstacles.base_tlight_threshold + config.obstacles.detection_speed_ratio * vehicle_speed
    ...
    def ignore_traffic_lights(self, active=True):
        ...
        config.obstacles.ignore_traffic_lights = active

    def ignore_stop_signs(self, active=True):
        """(De)activates the checks for stop signs"""
        config.obstacles.ignore_stop_signs = active

    def ignore_vehicles(self, active=True):
        """(De)activates the checks for stop signs"""
        config.obstacles.ignore_vehicles = active
    ...
    def lane_change(self, direction, same_lane_time=0, other_lane_time=0, lane_change_time=2):
        ...
        path : list = self._generate_lane_change_path(
            ...
            step_distance=config.unknown.sampling_resolution
        )
        if not path:
    ...
    def traffic_light_manager(self): # From behavior_agent.py
        ...
        affected, traffic_light = self._affected_by_traffic_light(self.lights_list, max_distance=config.obstacles.base_tlight_threshold)
    ...
    def _affected_by_traffic_light(self, lights_list=None, max_distance=None):
        ...
        """
        if config.obstacles.ignore_traffic_lights:
            return (False, None)
        ...
        if not max_distance:
            max_distance = config.obstacles.base_tlight_threshold
    ...
    def _vehicle_obstacle_detected(self, vehicle_list=None, max_distance=None, up_angle_th=90, low_angle_th=0, lane_offset=0):
        ...
        if config.obstacles.ignore_vehicles:
            return (False, None, -1)
        ...
        if not max_distance:
            max_distance = config.obstacles.base_vehicle_threshold
        ...
        use_bbs = config.unknown.use_bbs_detection or opposite_invasion or ego_wpt.is_junction
    ...
    def add_emergency_stop(self, control):
        ...
        control.brake = config.controls.max_brake
