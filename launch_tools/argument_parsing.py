...
class _SubparserMixIn(Protocol[_P]):
    ...
    @staticmethod
    def __call__(
        parser: Optional[argparse.ArgumentParser] = None, *args: _P.args, **kwargs: _P.kwargs
...
def subparser(func: Callable[Concatenate[argparse.ArgumentParser, _P], Any]) -> _SubparserMixIn[_P]:
    ...
    wrapper.add = (
        lambda parser, *args, **kwargs: wrapper(parser, *args, **kwargs)
        if isinstance(parser, argparse.ArgumentParser)
        else wrapper(parser(), *args, **kwargs)
    )
    return wrapper
...
def interactive_control_example(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--externalActor", action="store_true", help="attaches to externally created actor by role name"
    )
...
def automatic_control_example(argparser: argparse.ArgumentParser):
    ...
    argparser.add_argument(
        "-s", "--seed", help="Set seed for repeating executions (default: None)", default=None, type=int
    )
    argparser.add_argument(
        "-I", "--interactive", help="Enter interactive mode after initialization", action="store_true"
    )
    argparser.add_argument("-ap", "--autopilot", action="store_true", help="enable autopilot")
