...
def patch_rule():
    ...
    if ':exclude-members: "' in subcontent:
        ...
        subcontent = re.sub(
            r':exclude-members: "',
            ':special-members: __call__\n   :exclude-members: ", MultiRule, RandomRule, BlockingRule, ',
            subcontent,
            count=1,
        )
    else:
...
def remove_inheritance():
    ...
    files = ["agents.rules.obstacles.rst", "agents.rules.rst"]
    ...
    fine_files: dict[str, list[str]] = {
        "classes.rst": ["classes.constants", "classes.exceptions"],
...
def add_imported_members():
    modules: dict[str, "list[str] | str | Literal['all'] | tuple[str, list[str]]"] = {
        "classes": ["CustomSensorInterface"],
    ...
    for file, members in modules.items():
        ...
        if members != "all":
            ...
            if not members:
                continue
            subcontent = re.sub(
                pattern, extra + "   :imported-members: " + ", ".join(members) + extra + "\n", subcontent, count=1
            )
...
def remove_init():
    module: "dict[str, list[str | tuple[str, str | None]]]" = {
        "classes": [
            ("classes.evaluation_function", "__add__, __and__, __or__, __invert__"),
...
def patch_config_creation():
    ...
    if "no-special-members" in subcontent:
        return
    subcontent = re.sub(r":members:", r"\n   ".join([":members:", ":no-special-members:"]), string=subcontent, count=1)
...
def insert_file_references():
    rst_files = filter(
        lambda file: file not in ("index.rst", "readme_link.rst"), glob.glob("[!_]*.rst")
    )  # NOTE: if prefixed with ./ need to be stripped  # noqa: PTH207
...
def _no_value_constants():
    ...
    try:
        ...
        all_classes = [
            getattr(classes.constants, key)
            for key, cls in vars(classes.constants).items()
            if not key.startswith("_") and inspect.isclass(cls)
        ]
        flags = [  # noqa
            cls
            for cls in all_classes
            if issubclass(cls, (enum.Flag, enum.IntFlag, enum.Enum))
            and cls.__module__ == classes.constants.__name__
        ]
    ...
    with open("classes.rst", "r+") as f:
        ...
        subcontent = re.sub(r":members:", r"\n   ".join([":members:", ":no-value:"]), string=subcontent, count=1)
...
def patch_all():
    ...
    all_funcs: dict[str, "Callable"] = dict(
        filter(
