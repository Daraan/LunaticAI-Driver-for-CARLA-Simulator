...
def missing_reference_handle(
    app: "sphinx.application.Sphinx",
    env: "sphinx.environment.BuildEnvironment",
    node: "sphinx.addnodes.pending_xref",
    contnode: "docutils.nodes.Node",
) -> "docutils.nodes.Node | None":
    """
        ...
        if "reftarget" in node.attributes:
            ...
            if node.attributes.get("reftargetid") == "readme-workflow":
                ...
                newnode = docutils.nodes.reference(
                    ...
                    **node.attributes,
                    internal=True,
                    refid=node["reftargetid"],
                    refuri="../" * layers + reftarget + ".html#" + node["reftargetid"],
                )
                # breakpoint()
            ...
            if reftarget == "conf/":
                ...
                node["reftarget"] = "/".join((
                    REMOTE_URL,
                    "tree/main/conf/",
                ))  # if left untouched will point to local conf/
                node["classes"].extend(["github", "fa", "fa-github"])  # type: ignore[attr-defined]
                newnode = docutils.nodes.reference(
                    node.rawsource, "", *node.children, **node.attributes, internal=False, refuri=node["reftarget"]
                )
                node.replace_self(newnode)
                return
...
def source_read_listener(app: "sphinx.application.Sphinx", docname: str, content: list[str]):
    ...
    for line in content[:10]:
        if ":parser: myst_parser.sphinx_" in line and not (
            IGNORE_INCLUDE_MD and any("include:: _include.md" in line for line in content)
        ):
            global check_doctree
...
_patterns = {k: re.compile(rf"\b{re.escape(k)}(?<!{re.escape(v)})(\b|(?=\]|,|\)))") for k, v in _convert.items()}
            ...
            for keyword, typehint in obj.__annotations__.items():
                ...
                if isinstance(signature, str):
                    ...
                    if isinstance(signature.return_annotation, str):
                        signature.replace(
                            return_annotation=_patterns[replace_th].sub(new_hint, signature.return_annotation)
                        )
                elif isinstance(signature.parameters[keyword].annotation, str):
                    signature.parameters[keyword]._annotation = _patterns[replace_th].sub(
                        new_hint, signature.parameters[keyword]._annotation
