...
def main():
    ...
    while time.time() < t_end:
        try:
            ...
            # Brake logic for pedestrian crossing or sudden obstacle
            if matrix[i_car][j_car + 1] == 2:  # Pedestrian crossing
                # Check distance to pedestrian
                distance_to_pedestrian = j_car + 1
                if distance_to_pedestrian < MIN_DISTANCE_TO_BRAKE:
                    # Apply emergency braking
                    driver1.vehicle.setBrake(1)  # Adjust brake intensity as needed
                    print("Emergency braking activated! Pedestrian crossing!")
            elif matrix[i_car][j_car + 1] == 3:  # Sudden obstacle
                # Check distance to obstacle
                distance_to_obstacle = j_car + 1
                if distance_to_obstacle < MIN_DISTANCE_TO_BRAKE:
                    # Apply emergency braking
                    driver1.vehicle.setBrake(1)  # Adjust brake intensity as needed
                    print("Emergency braking activated! Sudden obstacle on the road!")
