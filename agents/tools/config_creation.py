...
class AgentConfig(DictConfigLike if TYPE_CHECKING else object):
        ...
        *,
        ...
        if settings is None:
            ...
            if assure_copy:
                ...
                behavior = settings()  # pyright: ignore[reportCallIssue]
            else:
    ...
    def check_config(
        cls,
        config: MT,
        strictness: "Literal[0, False]",
        as_dict_config: "Literal[False]",
    ) -> MT: ...
    ...
    def check_config(
        ...
        config: ConfigType,
        strictness: "Literal[0, False]",
        as_dict_config: "Literal[True]",
    ) -> ConfigType: ...

    @overload
    @classmethod
    def check_config(
        cls, config: ConfigType, strictness: int, as_dict_config: "Literal[True]"
    ) -> Self: ...
    ...
    if READTHEDOCS and not TYPE_CHECKING:
        ...
        check_config.__wrapped__.__annotations__ = check_config.__annotations__ = {
            "config": "NestedConfigDict",
            "strictness": "int",
            "as_dict_config": "bool",
            "return": "Self",
    ...
    @overload
    @classmethod
    def uses_overwrite_interface(
        cls, cls_: type[Self]
    ) -> "TypeGuard[type[ConfigWithOverwrites[Self]]]": ...

    @classmethod
    def uses_overwrite_interface(
        cls, cls_: type[Self] | None = None
    ) -> "TypeGuard[type[ConfigWithOverwrites[Self]]] | bool":
        """
                    ...
                    if key == "current_rule":
                        ...
                        for live_info_key in value.keys():
                            if _WARN_LIVE_INFO and not OmegaConf.is_missing(value, live_info_key):
                                logging.warning(
                                    "WARNING: live_info should only consist of missing values. Setting %s to %s",
                                    live_info_key,
                                    value[live_info_key],
                                )
                                setattr(live_info_dict, live_info_key, value[live_info_key])  # type: ignore[attr]
                    # Delegate False to a subfield
                    elif self.__dataclass_fields__[key].metadata.get("can_be_false", False) and value in (
                        False,
                        None,
                        "None",
                        True,
...
class BasicAgentPlannerSettings(AgentConfig):
    ...
    lateral_control_dict: PIDControllerDict = field(
        default_factory=lambda: PIDControllerDict(**{"K_P": 1.95, "K_I": 0.05, "K_D": 0.2})
    )
    """values of the lateral PID controller"""

    # NOTE: two variables because originally used with two different names in different places
    longitudinal_control_dict: PIDControllerDict = field(
        default_factory=lambda: PIDControllerDict(**{"K_P": 1.0, "K_I": 0.05, "K_D": 0})
    )
...
class LunaticAgentPlannerSettings(BehaviorAgentPlannerSettings):
    ...
    # NOTE: two variables because originally used with two different names in different places
    lateral_control_dict: PIDControllerDict = field(
        default_factory=partial(PIDControllerDict, **{"K_P": 1.95, "K_I": 0.05, "K_D": 0.2, "dt": II("${..dt}")})
    )
    """values of the lateral PID controller"""

    # NOTE: two variables because originally used with two different names in different places
    longitudinal_control_dict: PIDControllerDict = field(
        default_factory=partial(PIDControllerDict, **{"K_P": 1.0, "K_I": 0.05, "K_D": 0, "dt": II("${..dt}")})
    )
    """values of the longitudinal PID controller"""
        ...
        if AD_RSS_AVAILABLE:
            ...
            if not isinstance(self.use_stay_on_road_feature, RssRoadBoundariesModeAlias):  # pyright: ignore[reportUnnecessaryIsInstance]
                if isinstance(self.use_stay_on_road_feature, str):
                    ...
                    self.use_stay_on_road_feature = carla.RssRoadBoundariesMode.values[
                        bool(self.use_stay_on_road_feature)
                    ]
        else:
            if not isinstance(self.use_stay_on_road_feature, RssRoadBoundariesModeAlias):  # pyright: ignore[reportUnnecessaryIsInstance]
                if isinstance(self.use_stay_on_road_feature, str):
...
class LunaticAgentSettings(AgentConfig):
    ...
    detection_matrix: DetectionMatrixSettings = field(
        default_factory=DetectionMatrixSettings, init=False, metadata={"can_be_false": True}
    )
...
class CameraConfig(AgentConfig):
    ...
    camera_blueprints: list = field(
        default_factory=lambda: [  # pyright: ignore[reportRedeclaration]
    ...
    if TYPE_CHECKING:
        camera_blueprints: List["CameraBlueprint"] = field(
