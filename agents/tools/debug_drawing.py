...
def _draw_route_wp(
    ...
    * NOTE: This is based on the function from the Leaderboard_ project.
    ...
    world.debug.draw_point(
        waypoints[0][0].transform.location + carla.Location(z=vertical_shift),
        size=2 * size,  # type: ignore[arg-type]
        color=carla.Color(0, 0, 128),
        life_time=life_time,
    )
    world.debug.draw_point(
        waypoints[-1][0].transform.location + carla.Location(z=vertical_shift),
        size=2 * size,  # type: ignore[arg-type]
...
def _draw_route_trans(
    world: carla.World,
    waypoints: "list[tuple[carla.Transform, RoadOption]]",
    vertical_shift=0.5,
    size=0.3,
    downsample=1,
    life_time=1.0,
...
def draw_route(
    world: carla.World,
    waypoints: "list[tuple[carla.Transform | carla.Waypoint, RoadOption]]",
    vertical_shift=0.5,
    size=0.3,
    downsample=1,
    life_time=1.0,
...
def draw_waypoints(
    ...
    z=0.5,
    *,
    road_options: Optional["list[RoadOption]"] = None,
    **kwargs: Any,
) -> None:
    """
...
def debug_drawing(
    agent: "LunaticAgent", game_framework: "GameFramework", destination: Optional[carla.Location]
) -> None:
    """
    ...
    if traffic_light:
        ...
        game_framework.debug.draw_point(
            trigger_wp.transform.location + carla.Location(z=2), life_time=0.6, size=0.2, color=carla.Color(0, 0, 255)
        )
        ...
        draw_route(world_model.world, waypoints=[(wp, RoadOption.LEFT) for wp in affected_wps], size=0.1)


def draw_transform(debug: carla.DebugHelper, trans: carla.Transform, col: carla.Color = red, lt: float = -1) -> None:
    debug.draw_arrow(
        trans.location,
        trans.location + trans.get_forward_vector(),  # type: ignore[arg-type]
        thickness=0.05,
        arrow_size=0.1,
        color=col,
        life_time=lt,
...
def draw_junction(debug: carla.DebugHelper, junction: carla.Junction, l_time: float = 0.5):
    ...
    debug.draw_line(point1, point2, thickness=0.1, color=orange, life_time=l_time, persistent_lines=False)
    debug.draw_line(point2, point3, thickness=0.1, color=orange, life_time=l_time, persistent_lines=False)
    debug.draw_line(point3, point4, thickness=0.1, color=orange, life_time=l_time, persistent_lines=False)
    debug.draw_line(point4, point1, thickness=0.1, color=orange, life_time=l_time, persistent_lines=False)
    ...
    for pair_w in junction_w:
        ...
        debug.draw_point(pair_w[0].transform.location + carla.Location(z=0.75), 0.1, orange, l_time, False)
        draw_transform(debug, pair_w[1].transform, orange, l_time)
        debug.draw_point(pair_w[1].transform.location + carla.Location(z=0.75), 0.1, orange, l_time, False)
        debug.draw_line(
            pair_w[0].transform.location + carla.Location(z=0.75),
            pair_w[1].transform.location + carla.Location(z=0.75),
            0.1,
            white,
            l_time,
            False,
...
def draw_waypoint_info(debug: carla.DebugHelper, w: carla.Waypoint, lt: float = -1):
    ...
    debug.draw_string(w_loc + carla.Location(z=-0.5), str(w.lane_change), False, red, lt, False)


def draw_waypoint_union(
    debug: carla.DebugHelper, w0: carla.Waypoint, w1: carla.Waypoint, color: carla.Color = red, lt: float = 5
) -> None:
    debug.draw_line(
