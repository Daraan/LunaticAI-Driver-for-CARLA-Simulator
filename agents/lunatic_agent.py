...
class LunaticAgent(BehaviorAgent):
    ...
    def __init__(
        ...
        *,
        ...
        if world_model is None and len(sys.argv) > 1:
            logger.error(
                "BUG: Beware when not passing a WorldModel, the WorldModel currently ignores command line overrides, "
        ...
        )  # NOTE: THIS does not use self.config.planner.sampling_resolution
        if not self._global_planner:
            logger.error(
                "Global Route Planner not set - This should not happen, if the CarlaDataProvider has been initialized."
            )
            self._global_planner = GlobalRoutePlanner(
                CarlaDataProvider.get_map(), self.config.planner.sampling_resolution
            )
    ...
    def _set_collision_sensor(self):
        ...
        self._collision_sensor: carla.Sensor = assure_type(
            "carla.Sensor",
            CarlaDataProvider.get_world().spawn_actor(blueprint, carla.Transform(), attach_to=self._vehicle),
        )
    ...
    def _update_information(self, *, second_pass: bool = False):
        ...
        if not second_pass:
            ...
            # --- InformationManager ---
            information: InformationManager.Information = (
                self.information_manager.tick()
            )  # NOTE: # Warning: Currently not route-dependant, might need to be changed later
            ...
            self._look_ahead_steps = int(
                (self.live_info.current_speed_limit) / 10
            )  # TODO: Maybe make this an interpolation and make more use of it
        ...
        assert self.live_info.executed_direction == self._local_planner.target_road_option, (
            "Executed direction should not change."
        )
        ...
        if not self.done():
            ...
            self.live_info.incoming_waypoint, self.live_info.incoming_direction = (
                self._local_planner.get_incoming_waypoint_and_direction(  # pyright: ignore[reportAttributeAccessIssue]
                    steps=self._look_ahead_steps
                )
            )
        else:
            assert second_pass is False, (
                "In the second pass the agent should have replanned and agent.done() should be False"
            )
    ...
    def execute_phase(
        self, phase: Phase, *, prior_results: Any, update_controls: Optional[carla.VehicleControl] = None
    ) -> Context:
        """
        ...
        if self._validate_phases:
            assert (
                normal_next in {phase, Phase.USER_CONTROLLED}
        ...
        try:
            for rule in rules_to_check:  # todo: maybe dict? grouped by phase?
                assert self.current_phase in rule.phases, (
                    f"Current phase {self.current_phase} not in Rule {rule.phases}"
                )  # TODO remove:
                rule(self.ctx)
                # NOTE: Blocking rules can change the and above assertion will fail.
                if phase != self.current_phase:
                    logger.warning(
                        "Phase was changed by rule %s to %s. "
            ...
            print(
                "WARNING: A action likely tried to change `ctx.config` which is non-permanent. Use `ctx.agent.config.` instead."
            )
            raise
        return self.ctx
    ...
    def _plan_path_phase(self, *, second_pass: bool, debug: bool = False):
        try:
            ...
            if second_pass:
                logger.warning(
                    "UpdatedPathException was raised in the second pass. This should not happen: %s. Restrict your rule on ctx.second_pass.",
                    e,
                )
        ...
        try:
            ...
            if self.done():
                ...
                return self.run_step(
                    debug, second_pass=True
                )  # NOTE! For child classes like the leaderboard agent this calls the higher level run_step.
            ...
            # Handled Exceptions
                ...
                if second_pass > 5:
                    logger.warning(
                        "UpdatedPathException was raised more than %s times. Warning: This might be an infinite loop.",
                        second_pass,
                    )
                elif second_pass > 50:
                    raise RecursionError(
                        "UpdatedPathException was raised more than 50 times. Assuming an infinite loop and terminating"
                    ) from update_exception
                else:
                    logger.warning(
                        "%s was raised in the inner step, this should be done in Phase.PLAN_PATH instead.",
                        update_exception,
                    )
            ...
            ctx = self.execute_phase(
                Phase.RSS_EVALUATION | Phase.BEGIN, prior_results=None, update_controls=planned_control
            )
    ...
    def _inner_step(self, debug: bool = False) -> carla.VehicleControl:
        ...
        if vehicle_detection_result.obstacle_was_found:
            ...
            self.execute_phase(
                Phase.CAR_DETECTED | Phase.END, update_controls=control, prior_results=vehicle_detection_result
            )
            return self.get_control()  # type: ignore[return-value]
    ...
    def parse_keyboard_input(
        self, allow_user_updates: bool = True, *, control: Optional[carla.VehicleControl] = None
    ) -> None:
        """
    ...
    def apply_control(self, control: Optional[carla.VehicleControl] = None):
        ...
        if control is None:
            ...
            if control is None:
                raise ValueError(
                    "The agent has not yet performed a step this tick and has no control object was passed."
                )
    ...
    def pedestrian_avoidance_behavior(self) -> tuple[bool, ObstacleDetectionResult]:
        ...
        if detection_result.obstacle_was_found and (
            detection_result.distance
            - max(
                detection_result.obstacle.bounding_box.extent.y,  # pyright: ignore[reportOptionalMemberAccess]
    ...
    # @override
    def add_emergency_stop(
        self, control: carla.VehicleControl, reasons: "Optional[set[str]]" = None
    ) -> carla.VehicleControl:
        """
    ...
    def make_lane_change(
        ...
        for direction in order:
            ...
            if (
                can_change  # other_wpt is not None
                and lanes_have_same_direction(waypoint, other_wpt)  # type: ignore[arg-type]
                and other_wpt.lane_type == carla.LaneType.Driving
                ...
                if not detection_result.obstacle_was_found:
                    logger.debug(
                        "Change Lane, moving to the %s! Reason: %s",
                        direction,
                        "Overtaking" if tuple(order) == ("left", "right") else "Tailgating",
                    )

                    end_waypoint = self._local_planner.target_waypoint
                    # TODO: How to set waypoint order? Or better use generate_lane_change_path!
                    self.set_destination(
                        end_location=other_wpt.transform.location,  # type: ignore[arg-type]
                        start_location=end_waypoint.transform.location,
                        clean_queue=True,
                    )
                    return True
        return None
        ...
        *,
        ...
        if self._world_model.world_settings.synchronous_mode:
            ...
            if isinstance(config, DictConfig):  # pyright: ignore[reportUnnecessaryIsInstance]
                OmegaConf.select(config, "planner.dt", throw_on_missing=True)
            elif not isinstance(config.planner.dt, float):
                from omegaconf import MissingMandatoryValue

                raise MissingMandatoryValue(
                    "`config.planner.dt` needs to be set to a value. Cannot be:",
                    type(config.planner.dt),
                    config.planner.dt,
                )
        ...
        old_value = config_creation._WARN_LIVE_INFO  # pyright: ignore[reportPrivateUsage]
        ...
        config_creation._WARN_LIVE_INFO = old_value  # pyright: ignore[reportPrivateUsage]
    ...
    def set_vehicle(self, vehicle: carla.Actor) -> None:
        ...
        self._local_planner = DynamicLocalPlannerWithRss(
            self,
    ...
    def rss_set_road_boundaries_mode(
        self, road_boundaries_mode: Optional[Union[bool, RssRoadBoundariesModeAlias]] = None
    ) -> None:
    ...
    # @override
    def collision_and_car_avoid_manager(self, waypoint) -> NoReturn:  # type: ignore
        """
    ...
    # @override
    def _tailgating(self, waypoint, vehicle_list) -> NoReturn:  # type: ignore
