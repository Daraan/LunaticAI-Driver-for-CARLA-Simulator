...
class Another(Rule):
    ...
    actions = {
        True: lambda self, ctx: (_check_type(self, Rule), _check_type(ctx, Context)),
...
class CustomInitRule(Rule):
    def __init__(self, phases: Optional[Phase] = None):
        # NOTE: The
        super().__init__(
            phases or Phase.UPDATE_INFORMATION | Phase.BEGIN,
            condition=always_execute,
...
def _test_custom_init_Rule():
    ...
    if f"Warning 'condition' argument passed but class {CustomInitRule.__name__}" not in content:
        ...
        content = re.sub(
            rf"(WARNING:__main__:)?Warning 'condition' argument passed but class {CustomInitRule.__name__}.+?"
            "This might lead to undesired results.\n",
            "",
            content,
        )
    content = re.sub(
        f"(WARNING:__main__:)?Rules list is empty. {StoppedTooLongTrigger.__name__} will always return NOT_APPLICABLE.\n",
        "",
        content,
    )
...
cd_rule = CheckDescription(
    Phase.END,  # type: ignore[reportCallIssue]
    action=lambda ctx: _check_type(ctx, Context),
    condition=lambda self, _: (_check_type(self, Rule), _check_type(_, Context)),
)
