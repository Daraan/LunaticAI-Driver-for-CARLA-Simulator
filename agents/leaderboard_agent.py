...
class LunaticChallenger(AutonomousAgent, LunaticAgent):
    ...
    def setup(self, path_to_conf_file: Union[str, LaunchConfig]):
        ...
        if isinstance(path_to_conf_file, str):
            ...
            if not GameFramework.hydra_initialized():
                initialize_config_dir(
                    version_base=None, config_dir=os.path.abspath(config_dir), job_name="LeaderboardAgent"
                )
                ...
                args = cast(
                    ...
                    ),  # uses conf/agent/leaderboard
                )
                args.debug = DEBUG
                # Let scenario manager decide
                if args.map:
                    logger.warning(
                        f"Map should be set by scenario manager and be None in the config file found map is {args.map}."
                    )
        ...
        if self.args.pygame:
            self.controller = self.game_framework.make_controller(
    ...
    @staticmethod
    def _transform_to_waypoint(
        transform: "carla.Transform", project_to_road=True, lane_type=carla.LaneType.Driving
    ) -> "carla.Waypoint":
        # maybe move to misc / tools
        return CarlaDataProvider.get_map().get_waypoint(
            transform.location, project_to_road=project_to_road, lane_type=lane_type
    ...
    def _local_planner_set_plan(self, plan):
        ...
        if self.game_framework.launch_config.debug:
            draw_route(
                CarlaDataProvider.get_world(), plan, vertical_shift=0.5, size=0.15, downsample=1, life_time=1000.0
            )
    ...
    def set_global_plan(
        ...
        ds_ids: "list[int]" = downsample_route(
            global_plan_world_coord, DOWNSAMPLING_FACTOR_OF_ROUTE_COORDINATES
        )  # Downsample to less distance. TODO: should increase this
        ...
        self._global_plan_waypoints = [
            (self._transform_to_waypoint(transform), road_option)
